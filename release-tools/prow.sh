...
# - running the stable delphix example against a Kubernetes release
# - running the canary delphix example against a Kubernetes release
# - building the component in the current repo and running the
#   stable delphix example with that one component replaced against
...
# The delphix deployment script is searched for in several places.
#
...
#
# - CSI_PROW_DELPHIX_VERSION of the CSI_PROW_DELPHIX_REPO is checked
...
#   "none" disables the deployment of the delphix driver.
#
# When no deploy script is found (nothing in `deploy` directory,
# CSI_PROW_DELPHIX_REPO=none), nothing gets deployed.
configvar CSI_PROW_DELPHIX_VERSION "v0.0.1" "delphix driver"
configvar CSI_PROW_DELPHIX_REPO https://github.com/kubernetes-csi/csi-driver-host-path "delphix repo"
configvar CSI_PROW_DEPLOYMENT "" "deployment"
configvar CSI_PROW_DELPHIX_DRIVER_NAME "defs.csi.delphix.com" "the delphix driver name"

# If CSI_PROW_DELPHIX_CANARY is set (typically to "canary", but also
# "1.0-canary"), then all image versions are replaced with that
# version tag.
configvar CSI_PROW_DELPHIX_CANARY "" "delphix image"
...
configvar CSI_PROW_SANITY_SERVICE "delphix-service" "Kubernetes TCP service name that exposes csi.sock"
configvar CSI_PROW_SANITY_POD "csi-delphixplugin-0" "Kubernetes pod with CSI driver"
configvar CSI_PROW_SANITY_CONTAINER "delphix" "Kubernetes container with CSI driver"
...
find_deployment () {
    ...
    if [ "${CSI_PROW_DEPLOYMENT}" ]; then
        file="$dir/${CSI_PROW_DEPLOYMENT}/deploy-delphix.sh"
        if ! [ -e "$file" ]; then
            return 1
        fi
        echo "$file"
    ...
    # Ignore: See if you can use ${variable//search/replace} instead.
    # shellcheck disable=SC2001
    file="$dir/kubernetes-$(echo "${CSI_PROW_KUBERNETES_VERSION}" | sed -e 's/\([0-9]*\)\.\([0-9]*\).*/\1.\2/')/deploy-delphix.sh"
    if ! [ -e "$file" ]; then
        file="$dir/kubernetes-latest/deploy-delphix.sh"
        if ! [ -e "$file" ]; then
            return 1
        fi
    fi
...
# This installs the delphix driver example. CSI_PROW_DELPHIX_CANARY overrides all
# image versions with that canary version. The parameters of install_delphix can be
...
install_delphix () {
    ...
    if deploy_delphix="$(find_deployment "$(pwd)/deploy")"; then
        :
    elif [ "${CSI_PROW_DELPHIX_REPO}" = "none" ]; then
        return 1
    else
        git_checkout "${CSI_PROW_DELPHIX_REPO}" "${CSI_PROW_WORK}/delphix" "${CSI_PROW_DELPHIX_VERSION}" --depth=1 || die "checking out delphix repo failed"
        if deploy_delphix="$(find_deployment "${CSI_PROW_WORK}/delphix/deploy")"; then
            :
        else
            die "deploy-delphix.sh not found in ${CSI_PROW_DELPHIX_REPO} ${CSI_PROW_DELPHIX_VERSION}. To disable E2E testing, set CSI_PROW_DELPHIX_REPO=none"
        fi
    fi

    if [ "${CSI_PROW_DELPHIX_CANARY}" != "stable" ]; then
        images="$images IMAGE_TAG=${CSI_PROW_DELPHIX_CANARY}"
    fi
    # Ignore: Double quote to prevent globbing and word splitting.
    # It's intentional here for $images.
    # shellcheck disable=SC2086
    if ! run env $images "${deploy_delphix}"; then
        ...
        die "deploying the delphix driver with ${deploy_delphix} failed"
    fi
}
...
docker exec csi-prow-control-plane docker image ls --format='{{.Repository}} {{.Tag}} {{.ID}}' | grep -e csi -e delphix | while read -r repo tag id; do
...
# Whether the delphix driver supports raw block devices depends on which version
...
delphix_supports_block () {
    local result
    result="$(docker exec csi-prow-control-plane docker image ls --format='{{.Repository}} {{.Tag}} {{.ID}}' | grep delphix | while read -r repo tag id; do
        if [ "$tag" == "v1.0.1" ]; then
...
# driver test configuration for the delphix driver.
#
# The content depends on both what the E2E suite expects and what the
# installed delphix driver supports. Generating it here seems prone
...
SnapshotClass:
  FromName: true
DriverInfo:
  Name: ${CSI_PROW_DELPHIX_DRIVER_NAME}
  Capabilities:
    block: $(delphix_supports_block)
    persistence: true
    dataSource: true
    multipods: true
EOF
...
run_e2e () (
    ...
    # delphix driver was deployed and set ClientNodeName accordingly.
...
main () {
    ...
    if tests_need_kind; then
        ...
        if ${CSI_PROW_BUILD_JOB}; then
            ...
            # deploy-delphix.sh env variables for it. We also need to
        ...
        if tests_need_non_alpha_cluster; then
            ...
            if install_delphix "$images"; then
                collect_cluster_info
